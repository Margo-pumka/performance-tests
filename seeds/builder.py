...
class SeedsBuilder:
    ...
    def build_top_up_operation_result(self, account_id: str, card_id) -> SeedOperationResult:
        """
    ...
    def build_purchase_operation_result(self, account_id: str, card_id) -> SeedOperationResult:
        """
    ...
    def build_debit_card_account_result(self, plan: SeedAccountsPlan, user_id: str) -> SeedAccountResult:
        ...
        return SeedAccountResult(
            ...
            top_up_operations=[
                self.build_top_up_operation_result(account_id=account_id, card_id=card_id)
                for _ in range(plan.top_up_operations.count)
            ],
            purchase_operations=[
                self.build_purchase_operation_result(account_id=account_id, card_id=card_id)
                for _ in range(plan.purchase_operations.count)
            ],
            transfer_operations=[
                self.build_transfer_operation_result(account_id=account_id, card_id=card_id)
                for _ in range(plan.transfer_operations.count)
            ],
            cash_withdrawal_operations=[
                self.build_cash_withdrawal_operation_result(account_id=account_id, card_id=card_id)
